...
vector<int> Graph::route_node2roadSegment(vector<int> route_node){
    ...
    if (route_node.size() < 1)
        cout << "route size is smaller than 0 with " << route_node.size() << endl;
    route_roadSegment.reserve(route_node.size());
    int node01, node02, roadSegmentID, routeID;

    for (int i=0;i<route_node.size()-1;i++){
        node01 = route_node[i]; node02 = route_node[i+1];
       // if(map_nodeID_2_roadID.find(make_pair(node01,node02)) == map_nodeID_2_roadID.end())
            /*cout << node01 << "\t" << node02 << endl;*/
        roadSegmentID = map_nodeID_2_roadID[make_pair(node01,node02)];
        route_roadSegment.push_back(roadSegmentID);
    }
...
/*
    cout << "RoadSegment ID is: " << RoadSegmentID.first << endl;
*/
...
void Graph::one_route_parallel_update(vector<pair<pair<int, vector<int>>, int>> &multi_test, int i, int j, vector<Semaphore*>& vLock) 
{ 
...
//	cout << "i:" << i << "\tj: "<< j << endl;
	for (int k = i; k < j; k++)
	{
		pair<int, vector<int>> newRoute = multi_test[k].first;
		int inTime = multi_test[k].second;
...
void Graph::multi_route_update(vector<pair<pair<int, vector<int>>, int>> &multi_test){
    ...
    vLock.reserve(vRoad.size()); 
	cout << "vRoad size:" <<  vRoad.size() << endl;
	...
	int threadNum = 100;  
    int step = multi_test.size()/threadNum;
	vector<int> id_temp;
...
//		one_route_parallel_update(multi_test, i*step, (i+1)*step, vLock);
		
		if(i < threadNum - 1)
			threadf.add_thread(new boost::thread(&Graph::one_route_parallel_update, this, boost::ref(multi_test), i*step, (i+1)*step, boost::ref(vLock))); 
		else
			threadf.add_thread(new boost::thread(&Graph::one_route_parallel_update, this, boost::ref(multi_test), i*step, multi_test.size(), boost::ref(vLock))); 
//		break;
	}
    threadf.join_all();
...
vector<int> Graph::one_route_update(pair<int, vector<int>> newRoute, int inTime){
    ...
    // Step 1: 初始化储存受影响的roadSegmentID的集合，把newRoute的路段添加进去
    vector<pair<pair<int,int>, tuple<int, map<int, vector<vector<int>>>, vector<int>>>> roadSegmentSet; // RoadSegment, <0/leaveTime, I', D'>
    //if(newRoute.second.size() == 0)
    //    cout << "Empty newRoute" << endl;
    int RoadSegmentID1st = newRoute.second[0]; int count = 0;
...
pair<vector<pair<int, vector<int>>>, vector<int>> Graph::new_data_gener(
        vector<vector<int>> &route_data,  vector<vector<int>> &query_data, int ad_num)
{
    vector<vector<int>> new_path;
    new_path.reserve(ad_num);
    for (int i = 0; i < ad_num; i++)
    {
        ...
        new_path.push_back(route_data[k]);

/*
        cout << i << "\t" << new_path[i][0] << endl;
*/
    }

    vector<vector<int>> route_roadSegment;
    route_roadSegment.reserve(ad_num);
    for (int i = 0; i < new_path.size(); i++)
    {

/*
        cout << "before transfer is: " << new_path[i][0] << " and " << new_path[i][1] << endl;
*/

        route_roadSegment.push_back(route_node2roadSegment(new_path[i]));

/*
        cout << "after transfer is: " << route_roadSegment[i][0] << endl;
*/
    }

    vector<pair<int, vector<int>>> pNew_route;
    pNew_route.reserve(ad_num);
    for (int i = 0; i < route_roadSegment.size(); i++)
    {
        if(route_roadSegment[i].size() == 0)
            cout << "Empyt Route_roadSegment" << endl;
        pNew_route.push_back(make_pair(i+Pi.size(), route_roadSegment[i]));


/*
        cout << i << "\t" << route_roadSegment[i][0] << endl;
*/
    }

/*    for (int i = 0; i < route_data.size(); i++)
    {
        Pi.push_back(route_roadSegment[i]);

    }*/

    for (int i = 0; i < new_path.size(); i++)
    {
        Pi.push_back(new_path[i]);
    }
    ...
    {
        int k;
        if (i >= query_data.size())
            k = i % query_data.size();
        else
            k = i;

/*
        cout << i << "\t" << pNew_route[i].second.size() << endl;
*/

/*
        cout << i << "\t" << pNew_route[i].second[0] << endl;
*/

        depar_time = query_data[k][2];
        time_range_right = depar_time + 2*60;
        gener_depar_time = (rand() % (time_range_right - depar_time + 1)) + depar_time;
        depar_time_set.push_back(gener_depar_time);
    }

    return make_pair(pNew_route, depar_time_set);
}

void Graph::update_wo_threads(pair<vector<pair<int, vector<int>>>, vector<int>> new_data_pair, int num)
{
    vector<pair<int, vector<int>>> pNew_route = new_data_pair.first;
    vector<int> depar_time_set = new_data_pair.second;
