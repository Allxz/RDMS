...
//    vector<int> one_route_parallel_update(pair<int, vector<int>> newRoute, int inTime, vector<Semaphore*>& vLock);
	void one_route_parallel_update(vector<pair<pair<int, vector<int>>, int>> &multi_test, int i, int j, vector<Semaphore*>& vLock);
    ...
    pair<vector<pair<int, vector<int>>>, vector<int>> new_data_gener(
            vector<vector<int>> &route_data,  vector<vector<int>> &query_data, int ad_num);
    void update_wo_threads(pair<vector<pair<int, vector<int>>>, vector<int>> new_data_pair, int num);

    void ex_new_query(string ex_path, vector<int> new_query);
    void ex_new_route(string ex_path, vector<pair<int, vector<int>>> new_route);

    vector<int> read_new_query(string read_path, int read_num);
    vector<pair<int, vector<int>>> read_new_route(string read_path, int read_num, int exi_route_num);

    vector<pair<int, vector<int>>> read_new_route2(vector<vector<int>> &route_data, int ad_num);
};
