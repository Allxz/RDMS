...
/*    // 随机生成50000个new route和departure time
    ...
    // 生成并储存到路径
    g.data_generation(route_data, query_data, route_file, depar_file, Pi_file, 200);
...
/*    int gener_num = 200000;
    pair<vector<pair<int, vector<int>>>, vector<int>> new_data = g.new_data_gener(route_data, query_data,gener_num);*/
    string ex_query = g.Base + "new_data_20w/new_query_20w";
/*    // 写出20w条new query数据
    g.ex_new_query(ex_query, new_data.second);*/
    string ex_route = g.Base + "new_data_20w/new_route_20w";
/*    // 写出20w条new route数据
    g.ex_new_route(ex_route, new_data.first);*/

    // 读入制定数量的new route和new query，数量不能超过20w
    int new_num = 1000;
    vector<int> new_query = g.read_new_query(ex_query, new_num);
    vector<pair<int, vector<int>>> new_route = g.read_new_route2(route_data, new_num);
    //

/*
    vector<pair<int, vector<int>>> new_route = g.read_new_route(ex_route, new_num, read_num);
*/

    cout << "number of new route is: " << new_num << endl;
    // g.update_wo_threads(make_pair(new_route, new_query), new_num);

    // check the correctness of one_route_update by sequence.


    // g.one_route_update_check(route_data, query_data, new_num);


	// check the correctness of one_route_update by threads
    // vector<pair<pair<int, vector<int>>, int>> multi_test;
    // multi_test = g.multi_task_initial(new_route, new_query, new_num);

	
	vector<pair<pair<int, vector<int>>, int>> multi_test;
	for (int i = 0; i < new_query.size(); i++)
	{
		multi_test.push_back(make_pair(new_route[i], new_query[i]));
	}
