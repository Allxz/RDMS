...
void Graph::ex_new_query(string ex_path, vector<int> new_query)
{
    ofstream outfile;
    outfile.open(ex_path);

    for (int i = 0; i < new_query.size(); i++)
    {
        outfile << new_query[i] <<  " " << endl;
    }

    outfile.close();
}

void Graph::ex_new_route(string ex_path, vector<pair<int, vector<int>>> new_route)
{
    ofstream outfile;
    outfile.open(ex_path);
    ...
    for (int i = 0; i < new_route.size(); i++)
    {
        int route_id = new_route[i].first;
        int route_size = new_route[i].second.size();
        vector<int> route = new_route[i].second;

        outfile << route_id <<  " " << route_size << " ";
...
vector<int> Graph::read_new_query(string read_path, int read_num)
{
    ifstream IF(read_path);
    if(!IF){
        cout<<"Cannot open "<<read_path<<endl;
    }

    vector<int> new_query;

    new_query.reserve(read_num);

    int depar_time; int count_num = 0;

    while(IF >> depar_time and count_num < read_num)
    {
        new_query.push_back(depar_time);

        count_num += 1;
    }

    cout << "query size should be: " << read_num << " with size of query size is: " << new_query.size() << endl;

    return new_query;
}

vector<pair<int, vector<int>>> Graph::read_new_route(string read_path, int read_num, int exi_route_num)
{
    ifstream IF(read_path);
    if(!IF){
        cout<<"Cannot open "<<read_path<<endl;
    ...
    vector<pair<int, vector<int>>> new_route;
    new_route.reserve(read_num);

    int route_id; int route_size;
    vector<int> route; int vertex;
    int count_num = 0;

    while(IF >> route_id and count_num < read_num)
    {
        IF >> route_size;

        route.clear(); route.reserve(route_size);
        ...
        route_id = exi_route_num + count_num;
        new_route.push_back(make_pair(route_id, route));

        count_num += 1;
    }

    cout << "route size should be: " << read_num << " with size of route size is: " << new_route.size() << endl;

    return new_route;
}

vector<pair<int, vector<int>>> Graph::read_new_route2(vector<vector<int>> &route_data, int ad_num)
{
    ...
    cout << "query size should be:" <<  ad_num << "with size of query size is: " << pNew_route.size() << endl;
